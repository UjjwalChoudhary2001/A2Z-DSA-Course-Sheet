...
upper_bound(a,a+n,element)-a;
  */
  ...
  /* -->Lower and UpperBound functions are not present , all operations occur in
  O(1) time and O(n) in worst case.
  ...
  cout << "----------------------Map-----------------------------" << endl;
  /* It stores unique key value pairs and keys are in sorted order.*/
  // empty map container
  map<int, int> gquiz1;
